...
export default function NextSessionWidget({ session = {}, loading = false }) {
  const {
    title = 'No session',
    description = '',
    time = '—',
  ...
  return (
    <GlassCard className="flex flex-col">
      ...
      <div className="mt-6 space-y-2">
        <button
          className="w-full py-3 rounded-lg bg-[#D4FF00] text-black font-['Bebas_Neue'] text-lg tracking-wider hover:bg-[#c5ef00] transition disabled:opacity-50 disabled:cursor-not-allowed"
          disabled={status === 'EMPTY'}
        >
          START SESSION
        </button>
        <button
          className="w-full py-3 rounded-lg border border-[#1A1A1A] text-gray-300 font-['Bebas_Neue'] tracking-wider hover:border-gray-600 transition disabled:opacity-50 disabled:cursor-not-allowed"
          disabled={status === 'EMPTY'}
        >
          RESCHEDULE
        </button>
